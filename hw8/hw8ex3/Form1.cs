...
namespace hw8ex3
{
    public partial class Form1 : Form
    {
        ...
        private void tsmiSave_Click(object sender, EventArgs e)
        {
            ...
            else MessageBox.Show("Data base isn't created");
        }
        ...
        private void nudNumber_ValueChanged(object sender, EventArgs e)
        {
            try
            {
                rtbQuestion.Text = database[(int)nudNumber.Value - 1].Text;
                cbTrue.Checked = database[(int)nudNumber.Value - 1].TrueFalse;
            }
            catch (NullReferenceException ex)
            {
                MessageBox.Show($"Details: {ex.Message}", "Question isn't there");
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                database[(int)nudNumber.Value - 1].Text = rtbQuestion.Text;
                database[(int)nudNumber.Value - 1].TrueFalse = cbTrue.Checked;
            }
            catch (NullReferenceException ex)
            {
                MessageBox.Show($"Details: {ex.Message}", "Open or create file with questions");
            }
        }
